...
<!--t0803-->
InputLib 与上层应用的边界是性能，InputLib 负责性能敏感的输入分发，上层则只负责执行动作。
...
- 允许 up 已按下的任意键
  ...
  `Ctrl{K{} F{}}`
  `Ctrl{K{F{}}}`
- ~~除了下个event涉及到的键外的所有的键的重置点~~
- 把 RELEASE_ANY 的功能转移给 key_state_specifier？
  感觉会引发更大的混乱，而且会影响 `^`。

重置点实际上有意义吗？
如果空 specifier 表示没有其它键的话，`{Ctrl}{A}` 就没法用了。
这样的话，为了不误识别 `{A}`，就必须在开头加入重置点，但这样又需要想办法允许不加入重置点。
让 specifier 同时表示“允许前面按下这些键”，除了修复重置点带来的问题，另一方面也可以起到化简事件描述的作用。

VSC block
- BREAK_BLOCK 命令
  但命令名和 `^` 写在一起看起来不太好看，允许先 break，再提供参数？还是单独一个参数块，从而允许在开头提供参数？
  `命令{打开项目1} Ctrl+K`，但这样不允许多个命令
  `命令 打开项目1{Ctrl+K}`，但这样又不允许在结尾设置命令，而且也不便于编写复杂命令
  还是暂时不考虑这个了，`设备{}` 这种功能更有价值。
  不，融合命令的真正价值就在于可以嵌套条件，比如 `设备{A} Ctrl+A 命令{B}`。
  但问题是，我的 `{}` 并不代表作用域，这种语法看起来很奇怪。
  设备或许可以改成 `设备 A{}`，但 `if {}` 呢？
  `if cond {}` 和 `when {cond}`？